...
Entcoin v1.5.5 is a compact proof-of-work mainnet packaged as a Windows and
...
> activated at block 125555. Every v1.2.0 node and miner must install v1.5.5.

## What v1.5.5 includes
...
- A mandatory height-125555 consensus upgrade that replaces the asymmetric
  epoch DAA with a 600-second integer ASERT rule while preserving all historical
  version-1 blocks, chain identity, wallets, balances, and storage.
...
needed. The current v1.5.5 release may be unsigned, so Windows SmartScreen may show
...
Consensus v2 activation           block 125,555
Consensus v2 DAA half-life        600 seconds
```
...
`.entwallet` backup. Start v1.5.5 to create the mainnet directory, then use the
...
Prerequisites: Go 1.26.6, Node.js 22+, and Wails v2.13.0. Windows packaging
